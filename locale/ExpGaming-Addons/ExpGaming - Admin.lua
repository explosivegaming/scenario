...
Event.register(defines.events.on_gui_click, function(event)
	...
	if element.valid and element.type == 'radiobutton' and element.parent.name == 'modifiers_table' then
		...
		for n,effect in pairs(modifier[1]) do
			local base = nil
			if modifier[3] and modifier[3][n] then base = global.modifiers.base[modifier[3][n]]
			else if is_player then base = player[effect]/old_slected else base = force[effect]/old_slected end
				if base == 0 then modifier[3] = {effect} global.modifiers.base[effect] = 0 end
				if base == 0 or base == nil then base = 1 slected=slected-1 end end
			base = base or 0
			local new_value = base*slected
