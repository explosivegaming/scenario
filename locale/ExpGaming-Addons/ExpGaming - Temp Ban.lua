...
define_command('temp-ban','Temp ban a player untill the next reset, can not be un done',{'player','reason',true},'Mod',function(player,event,args)
    if player == '<server>' then
        ...
        rank_print(jail_player..' has been temp banned by '..player..' reason: '..table.concat(args,' ',2),'Guest')
    else
        local jail_player = game.players[args[1]]
        ...
        ExpGui.draw_frame.popup('temp_ban',{player.name,table.concat(args,' ',2)})
        rank_print(jail_player..' has been temp banned by '..player.name..' reason: '..table.concat(args,' ',2),'Guest')
    end
end)
