...
define_command('kill','Used to kill a player, you can always kill your self!',{'player'},function(player,event,args)
    if player == '<server>' then
        ...
        if kill_player == player or get_rank(player).power < get_rank(kill_player).power and get_rank(player).power <= string_to_rank_group('Moderation').lowest_rank.power then
